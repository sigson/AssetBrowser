...
const DIRECT_TEX_TYPES = {
    "Texture2D": true, "CompressedTexture2D": true, "ImageTexture": true,
    "AtlasTexture": true, "PortableCompressedTexture2D": true, "CanvasTexture": true,
    "Cubemap": true
}
...
func is_direct_texture(res_type):
    if res_type == null or res_type == "":
        return false
    if DIRECT_TEX_TYPES.has(res_type):
        return true
    # Любой движковый подкласс Texture2D (CompressedTexture2D для импортированных
    # png/jpg/svg/webp, ImageTexture, AtlasTexture, NoiseTexture2D, и т.д.).
    return ClassDB.class_exists(res_type) and ClassDB.is_parent_class(res_type, "Texture2D")
...
func get_type_icon(n):
    ...
    if _base != null and _base.has_theme_icon(t, "EditorIcons"):
        return _base.get_theme_icon(t, "EditorIcons")
    return _icon_or("File")

func _icon_or(name):
    if _base != null and _base.has_theme_icon(name, "EditorIcons"):
        return _base.get_theme_icon(name, "EditorIcons")
    return null

func previewable(res_type):
    if res_type == null or res_type == "":
        return false
    if is_direct_texture(res_type):
        return true
    if res_type == "TileSet" or res_type == "PackedScene":
        return true
    # Движок умеет генерировать превью для мешей и материалов (любых подклассов).
    if ClassDB.class_exists(res_type):
        if ClassDB.is_parent_class(res_type, "Mesh"):
            return true
