...
func init(editor, settings):
    ...
    if event_driven:
        _fs.connect("filesystem_changed", self, "on_fs_changed")
        _fs.connect("resources_reimported", self, "on_reimported")
        _fs.connect("sources_changed", self, "on_sources_changed")
    _prev.connect("preview_invalidated", self, "on_preview_invalidated")
    _connected = true
...
func shutdown():
    if not _connected:
        return
    if _fs != null:
        if _fs.is_connected("filesystem_changed", self, "on_fs_changed"):
            _fs.disconnect("filesystem_changed", self, "on_fs_changed")
        if _fs.is_connected("resources_reimported", self, "on_reimported"):
            _fs.disconnect("resources_reimported", self, "on_reimported")
        if _fs.is_connected("sources_changed", self, "on_sources_changed"):
            _fs.disconnect("sources_changed", self, "on_sources_changed")
    if _prev != null and _prev.is_connected("preview_invalidated", self, "on_preview_invalidated"):
        _prev.disconnect("preview_invalidated", self, "on_preview_invalidated")
    _connected = false
...
func get_node_at(path):
    ...
    if _dir_exists(path):
        ...
        return n
    var parent = path.substr(0, path.find_last("/"))
