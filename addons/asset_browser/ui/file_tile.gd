...
func build(view, node, cell, shared):
    ...
    custom_minimum_size = Vector2(cell + 8, cell + 26)
    ...
    _icon.custom_minimum_size = Vector2(cell, cell)
    box.add_child(_icon)

    _label = Label.new()
    _label.text = node.name
    _label.align = HORIZONTAL_ALIGNMENT_LEFT
    _label.clip_text = true
    _label.mouse_filter = Control.MOUSE_FILTER_IGNORE
    _label.custom_minimum_size = Vector2(0, 18)
    ...
    if mb.button_index == MOUSE_BUTTON_LEFT:
        if mb.pressed:
            if mb.double_click:
                ...
                _view.tile_pressed(_node.path, mb.ctrl_pressed, mb.shift_pressed)
        else:
