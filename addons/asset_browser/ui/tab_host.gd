...
func _configure_timers():
    ...
    if want_scan:
        if _scan_timer == null or not is_instance_valid(_scan_timer):
            ...
            _scan_timer.connect("timeout", Callable(self, "on_scan_tick"))
    ...
    if want_thumb:
        if _thumb_timer == null or not is_instance_valid(_thumb_timer):
            ...
            _thumb_timer.connect("timeout", Callable(self, "on_thumb_tick"))
...
func _rebuild_tabs_row():
    ...
    for i in range(_states.size()):
        ...
        btn.button_pressed = (i == _active)
        btn.focus_mode = Control.FOCUS_NONE
        btn.connect("pressed", Callable(self, "on_tab_pressed").bindv([i]))
        ...
        if _states.size() > 1:
            ...
            x.connect("pressed", Callable(self, "on_tab_close").bindv([i]))
            tab_box.add_child(x)
    ...
    plus.connect("pressed", Callable(self, "on_new_tab"))
...
func show_placeholder():
    ...
    lbl.align = HORIZONTAL_ALIGNMENT_CENTER
    box.add_child(lbl)

    var btn = Button.new()
    btn.text = "Resume"
    btn.connect("pressed", Callable(self, "on_resume_pressed"))
    box.add_child(btn)
