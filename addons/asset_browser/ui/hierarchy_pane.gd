...
func build(shared):
    ...
    _tree.connect("item_selected", Callable(self, "on_selected"))
    _tree.connect("item_mouse_selected", Callable(self, "on_rmb"))
    _tree.connect("item_collapsed", Callable(self, "on_item_collapsed"))
    add_child(_tree)

    _ctx_menu = PopupMenu.new()
    _ctx_menu.connect("id_pressed", Callable(self, "on_ctx_menu"))
    add_child(_ctx_menu)

    _del_dlg = ConfirmationDialog.new()
    _del_dlg.title = "Delete folder"
    _del_dlg.connect("confirmed", Callable(self, "on_delete_confirmed"))
    add_child(_del_dlg)

    _refresh_timer = Timer.new()
    _refresh_timer.one_shot = true
    _refresh_timer.connect("timeout", Callable(self, "on_refresh_timer"))
    add_child(_refresh_timer)

    _s.favorites.connect("changed", Callable(self, "on_favorites_changed"))
...
func _full_rebuild():
    ...
    if fav_dirs.size() > 0:
        ...
        for f in fav_dirs:
            ...
            it.set_text(0, f.substr(f.rfind("/") + 1) + "/")
...
func _find_item(from, dir):
    if from == null:
        return null
    var child = from.get_first_child()
    while child != null:
...
func on_rmb(_pos, mb = MOUSE_BUTTON_RIGHT):
