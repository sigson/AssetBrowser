...
func build(shared, state):
    ...
    _repop_timer.connect("timeout", Callable(self, "populate"))
...
func _build_create_menu():
    ...
    _create_menu.connect("id_pressed", Callable(self, "on_create_picked"))
...
func _sort_into(list):
    ...
    list.sort_custom(Callable(self, "_cmp_nodes"))
    for n in list:
        _nodes.append(n)
...
func _build_list():
    ...
    _list.connect("item_activated", Callable(self, "on_list_activate"))
    _list.connect("multi_selected", Callable(self, "on_list_multi"))
    _list.connect("item_mouse_selected", Callable(self, "on_list_rmb"))
    _list.connect("empty_clicked", Callable(self, "on_list_empty_rmb"))
    _list.connect("nothing_selected", Callable(self, "on_list_nothing_selected"))
    _list.connect("column_title_clicked", Callable(self, "on_sort_column"))
    _list.connect("gui_input", Callable(self, "on_list_gui"))
    _list.set_drag_forwarding(
        Callable(self, "get_drag_data_fw"),
        Callable(self, "can_drop_data_fw"),
        Callable(self, "drop_data_fw"))
    _list.set_column_expand(0, false)
    _list.set_column_custom_minimum_width(0, max(50, _state.col_name))
    _list.set_column_expand(1, false)
    _list.set_column_custom_minimum_width(1, max(30, _state.col_type))
    ...
    for n in _nodes:
        ...
        it.set_text_alignment(2, HORIZONTAL_ALIGNMENT_CENTER)
...
func _build_grid():
    ...
    if _virtual:
        ...
        _grid.add_theme_constant_override("h_separation", _gap_px)
        _grid.add_theme_constant_override("v_separation", _gap_px)
...
func _update_canvas_size():
    ...
    _canvas.custom_minimum_size = Vector2(0, rows * _row_stride)

func _compute_columns(cell, gap):
    var w = size.x
    if w <= 1:
        w = custom_minimum_size.x if custom_minimum_size.x > 1 else 200
    w -= 16.0
...
func _make_tile(i):
    ...
    if _virtual and _canvas != null:
        ...
        tile.position = Vector2(col * _col_stride, row * _row_stride)
        tile.size = Vector2(_cell_px + 8, _cell_px + 26)
...
func _notification(what):
    ...
    if _virtual and _canvas != null:
        ...
        for key in _tiles.keys():
            ...
            var row = i / max(1, _cols)
            var col = i % max(1, _cols)
            t.position = Vector2(col * _col_stride, row * _row_stride)
        _win_first = -1
        _win_last = -1
...
func _emit_selection(last):
    emit_signal("selection_changed", PackedStringArray(_selection))
...
func _clear_selection():
    ...
    emit_signal("selection_changed", PackedStringArray())
...
func get_selection():
    return PackedStringArray(_selection)
...
func on_list_rmb(_pos, mb = MOUSE_BUTTON_RIGHT):
    _sync_list_selection()
    if mb == MOUSE_BUTTON_RIGHT:
        popup_context()

func on_list_empty_rmb(_pos, mb = MOUSE_BUTTON_RIGHT):
    if mb != MOUSE_BUTTON_RIGHT:
        return
    if not _state.search.is_empty():
        return
    _create_menu.popup(Rect2i(DisplayServer.mouse_get_position(), Vector2i.ZERO))

func on_list_nothing_selected():
    _clear_selection()

func on_sort_column(col, _mb = MOUSE_BUTTON_LEFT):
...
func _build_context_menu():
    ...
    _ctx.connect("id_pressed", Callable(self, "on_context"))
    add_child(_ctx)

func popup_context():
    _ctx.popup(Rect2i(DisplayServer.mouse_get_position(), Vector2i.ZERO))
...
func on_context(id):
    ...
    match id:
        ...
        9:
            if target != null and DirAccess.dir_exists_absolute(target):
...
func _build_dialogs():
    ...
    _del_dlg.title = "Delete"
    _del_dlg.connect("confirmed", Callable(self, "on_delete_confirmed"))
    add_child(_del_dlg)

    _ren_dlg = AcceptDialog.new()
    _ren_dlg.title = "Rename"
    _ren_dlg.min_size = Vector2i(280, 90)
    _ren_dlg.get_ok_button().text = "OK"
    _ren_dlg.connect("confirmed", Callable(self, "on_rename_ok"))
    var vb = VBoxContainer.new()
    vb.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT, Control.PRESET_MODE_KEEP_SIZE, 8)
    _ren_input = LineEdit.new()
    _ren_input.connect("text_submitted", Callable(self, "on_rename_entered"))
...
func begin_rename(path):
    _ren_target = path
    _ren_input.text = path.substr(path.rfind("/") + 1)
...
func on_rename_ok():
    ...
    if new_name.length() == 0:
        _ren_dlg.hide()
        return
    var dir = _ren_target.substr(0, _ren_target.rfind("/"))
...
func _gui_input(ev):
    ...
    if ev is InputEventKey and ev.pressed:
        if ev.keycode == KEY_F2 and _selection.size() == 1:
            begin_rename(_selection[0])
            accept_event()
        elif ev.keycode == KEY_DELETE and _selection.size() > 0:
            _confirm_delete()
            accept_event()
        elif ev.ctrl_pressed and ev.keycode == KEY_C:
            _s.clipboard.set_data(_selection.duplicate(), ABClipboardModel.ClipboardMode.COPY)
            accept_event()
        elif ev.ctrl_pressed and ev.keycode == KEY_X:
            _s.clipboard.set_data(_selection.duplicate(), ABClipboardModel.ClipboardMode.CUT)
            accept_event()
        elif ev.ctrl_pressed and ev.keycode == KEY_V:
            paste()
            accept_event()
        elif ev.ctrl_pressed and ev.keycode == KEY_D:
            for p in _selection:
                ABFileOps.duplicate_path(p)
            _rescan()
            accept_event()
...
func can_drop_data_fw(_pos, data):
    return is_files_data(data)

func drop_data_fw(_pos, data):
...
func _make_drag_data():
    var paths = PackedStringArray(_selection) if _selection.size() > 0 else PackedStringArray()
...
func _size_of(n):
    ...
    if n.size >= 0:
        return n.size
    var f = FileAccess.open(n.path, FileAccess.READ)
    if f == null:
        n.size = 0
        return 0
    n.size = int(f.get_length())
    f.close()
    return n.size
