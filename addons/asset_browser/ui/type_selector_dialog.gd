...
func build(shared):
    ...
    window_title = "Filter by resource type"
    resizable = true
    rect_min_size = Vector2(440, 560)
    get_ok().text = "Apply"
    ...
    vb.rect_min_size = Vector2(420, 500)
    add_child(vb)

    _search = LineEdit.new()
    _search.placeholder_text = "search type... (e.g. ani, texture)"
    _search.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    _search.connect("text_changed", self, "on_search_text")
    ...
    _tree.connect("button_pressed", self, "on_button_pressed")
    _tree.connect("item_activated", self, "on_item_activated")
    vb.add_child(_tree)
    ...
    connect("custom_action", self, "on_custom_action")
...
func open(current_filter):
    ...
    popup_centered(Vector2(460, 580))
    _search.grab_focus()
...
func on_button_pressed(item_obj, column, _id):
    if column != 0:
        return
