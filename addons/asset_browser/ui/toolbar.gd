...
func build(shared, state):
    ...
    _back.connect("pressed", Callable(self, "on_back"))
    row.add_child(_back)

    _fwd = _nav_button("Forward", ">", "Forward")
    _fwd.connect("pressed", Callable(self, "on_fwd"))
    row.add_child(_fwd)

    _up = _nav_button("ArrowUp", "^", "Parent folder")
    _up.connect("pressed", Callable(self, "on_up"))
    row.add_child(_up)

    _create = MenuButton.new()
    _create.tooltip_text = "Create"
    ...
    pm.connect("id_pressed", Callable(self, "on_create"))
    row.add_child(_create)

    _sort = MenuButton.new()
    _sort.tooltip_text = "Sort"
    ...
    sp.connect("id_pressed", Callable(self, "on_sort_pick"))
    sp.connect("about_to_popup", Callable(self, "refresh_sort_checks"))
    row.add_child(_sort)

    var gear = Button.new()
    gear.tooltip_text = "View settings"
    ...
    if gear_ic != null:
        ...
        gear.text = "cfg"
    gear.connect("pressed", Callable(self, "on_settings"))
    row.add_child(gear)

    _freeze = Button.new()
    _freeze.toggle_mode = true
    _freeze.tooltip_text = "Freeze current items"
    ...
    if frz_ic != null:
        ...
        _freeze.text = "hold"
    _freeze.connect("pressed", Callable(self, "on_freeze"))
    row.add_child(_freeze)

    _kill = Button.new()
    _kill.toggle_mode = true
    _kill.tooltip_text = "Suspend / resume Asset Browser"
    ...
    if kill_ic != null:
        ...
        _kill.text = "off"
    _kill.button_pressed = _s != null and _s.lifecycle != null and _s.lifecycle.is_suspended
    _kill.connect("pressed", Callable(self, "on_killswitch"))
    row.add_child(_kill)

    _restart = Button.new()
    _restart.tooltip_text = "Restart Asset Browser plugin (rebuild after recompile)"
    ...
    _restart.connect("pressed", Callable(self, "on_restart"))
    row.add_child(_restart)

    if _s != null and _s.lifecycle != null:
        _s.lifecycle.connect("suspended_changed", Callable(self, "on_suspended_changed"))
    ...
    crumbs.connect("segment_picked", Callable(self, "on_segment"))
    row.add_child(crumbs)

    search = ABSearchBar.new()
    search.build(_s, state)
    search.connect("search_changed", Callable(self, "on_search"))
    add_child(search)
...
func on_freeze():
    emit_signal("freeze_toggled", _freeze.button_pressed)

func set_freeze_visual(on):
    if _freeze != null:
        _freeze.button_pressed = on

func on_suspended_changed(suspended):
    if _kill != null:
        _kill.button_pressed = suspended
