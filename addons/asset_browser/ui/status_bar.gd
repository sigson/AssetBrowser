...
func build(preview_size, max_preview_size):
    ...
    _note = Label.new()
    _note.text = ""
    _note.add_theme_color_override("font_color", Color(0.95, 0.75, 0.2))   # приглушённо-янтарный
    add_child(_note)
    ...
    _zoom.custom_minimum_size = Vector2(70, 0)
    _zoom.size_flags_vertical = Control.SIZE_SHRINK_CENTER
    _zoom.connect("value_changed", Callable(self, "on_zoom"))
    add_child(_zoom)
