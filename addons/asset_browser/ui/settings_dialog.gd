...
func build(settings):
    _s = settings
    title = "Asset Browser Settings"
    min_size = Vector2i(460, 620)

    var scroll = ScrollContainer.new()
    scroll.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT, Control.PRESET_MODE_KEEP_SIZE, 10)
    ...
    apply.text = "Apply"
    apply.connect("pressed", Callable(self, "on_apply"))
    vb.add_child(apply)

func _header(parent, text):
    var l = Label.new()
    l.text = text
    l.add_theme_color_override("font_color", Color(0.55, 0.7, 1))
...
func _add_profile_btn(row, text, p):
    ...
    b.connect("pressed", Callable(self, "on_profile").bindv([p]))
    row.add_child(b)
...
func on_apply():
    var e = [
        ...
        ["scan", "poll_only_when_visible", _poll_when_visible.button_pressed],
        ["tree", "lazy_subdirs", _lazy_subdirs.button_pressed],
        ...
        ["files", "virtualize_grid", _virtualize.button_pressed],
        ["files", "repopulate_min_interval_ms", int(_repop_ms.value)],
        ["files", "defer_size_column", _defer_size.button_pressed],
        ["search", "async", _search_async.button_pressed],
        ...
        ["search", "content_requires_global_optin", _content_opt_in.button_pressed],
        ...
        ["preview", "only_visible", _preview_only_visible.button_pressed],
        ...
        ["lifecycle", "persist_suspended", _persist_suspended.button_pressed],
        ["layout", "mode", LAYOUT_MODES[_layout.selected]],
        ["nav", "show_parent_folder", _show_parent.button_pressed],
